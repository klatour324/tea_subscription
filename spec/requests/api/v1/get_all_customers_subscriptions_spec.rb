...
RSpec.describe "Get all customer's subscriptions" do
  describe 'happy path' do
    before :each do
      ...
      @subscription3 = Subscription.create!(title: "Monthly Menthe", price: 49.99, frequency: "Twice a month", customer_id: @customer.id, tea_id: @tea.id, status: 'cancelled')
    end
    ...
    it "can fetch all of a customer's subscriptions and return both active and cancelled subscriptions" do
      ...
      expect(result[:data].first[:attributes].keys).to eq([:title, :price, :status, :frequency, :customer_id, :tea_id,] )
